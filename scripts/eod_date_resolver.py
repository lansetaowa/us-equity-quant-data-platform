...
def _as_local_timestamp(value: datetime, timezone: str) -> pd.Timestamp:
    """
    Convert datetime into timezone-aware pandas Timestamp in the target timezone.
    """
...
def _schedule_around(
    ...
    A 14-day lookback is enough for normal weekends and holidays while staying
    simple for Day 1.
    """
    ...
    if schedule.empty:
        raise ValueError(
            f"No market sessions found for calendar={calendar.name} "
